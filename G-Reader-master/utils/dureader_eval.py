...
def compute_bleu_rouge(pred_dict, ref_dict, bleu_order=4):
    ...
    rouge_score, rouge_values = Rouge().compute_score(ref_dict, pred_dict)
    scores['Rouge-L'] = rouge_score
    return scores, rouge_values


def very_compute_bleu_rouge(pred_dict, ref_dict, bleu_order=4):
    """
    Compute best bleu and rouge scores.
    """
    assert set(pred_dict.keys()) == set(ref_dict.keys()), \
            "missing keys: {}".format(set(ref_dict.keys()) - set(pred_dict.keys()))
    result_rouge_scores = {}
    ...
    for key in ref_dict.keys():
        bleu_scores, bleu_values = Bleu(bleu_order).compute_score(ref_dict[key], pred_dict[key])

        best_score = 0
        worst_score = 1
        for bleu_val in bleu_values[3]:
            every_bleu_4_scores.append(bleu_val)
            if bleu_val > best_score:
                best_score = bleu_val
            if bleu_val < worst_score:
                worst_score = bleu_val
        best_bleu_4_scores.append(best_score)
        worst_bleu_4_scores.append(worst_score)

        rouge_score, rouge_values = Rouge().compute_score(ref_dict[key], pred_dict[key])
        best_score = 0
        worst_score = 1
        for rouge_val in rouge_values:
            every_rouge_scores.append(rouge_val)
            if rouge_val > best_score:
                best_score = rouge_val
            if rouge_val < worst_score:
                worst_score = rouge_val
        best_rouge_scores.append(best_score)
        worst_rouge_scores.append(worst_score)
    average_best_rouge_score = np.mean(np.array(best_rouge_scores))
    average_worst_rouge_score = np.mean(np.array(worst_rouge_scores))
    average_avg_rouge_score = np.mean(np.array(every_rouge_scores))

    average_best_bleu_score = np.mean(np.array(best_bleu_4_scores))
    average_worst_bleu_score = np.mean(np.array(worst_bleu_4_scores))
    average_avg_bleu_score = np.mean(np.array(every_bleu_4_scores))

    result_rouge_scores['Best-Rouge-L'] = average_best_rouge_score
    result_rouge_scores['Avg-Rouge-L'] = average_avg_rouge_score
    result_rouge_scores['Worst-Rouge-L'] = average_worst_rouge_score

    result_bleu_scores['Best-Bleu-4'] = average_best_bleu_score
    result_bleu_scores['Avg-Bleu-4'] = average_avg_bleu_score
    result_bleu_scores['Worst-Bleu-4'] = average_worst_bleu_score
    return result_rouge_scores, result_bleu_scores


def rabel_compute_bleu_rouge(pred_dict, ref_dict, bleu_order=4):
    """
    Compute best bleu and rouge scores.
    """
    assert set(pred_dict.keys()) == set(ref_dict.keys()), \
    ...
    for key in ref_dict.keys():
        ...
        second_score = 0
        index = 0
        for rouge_id, rouge_val in enumerate(rouge_values):
            if rouge_val > best_score:
                second_score = best_score
                best_score = rouge_val
                index = rouge_id
        result.append({'question_id': key, "best_answer_index": index})
    return result


def rouge_compute_bleu_rouge(pred_dict, ref_dict, bleu_order=4):
    """
    Compute best bleu and rouge scores.
    """
    assert set(pred_dict.keys()) == set(ref_dict.keys()), \
    ...
    for key in ref_dict.keys():
        if len(pred_dict[key]) == 1:
            the_mat = [str(0), str(0), str(0), str(0), str(0), str(0), str(0), str(0)]
            result.append({'question_id': key, "ans_id": 0, 'the_mat': the_mat})
            continue

        for ans_id in ref_dict[key].keys():
            the_mat = []

            rouges = []
            bleus = []
            for ref_ans in ref_dict[key][ans_id]:
                the_ref, the_ans = defaultdict(list), defaultdict(list)
                the_ans[ans_id] = pred_dict[key][ans_id]
                the_ref[ans_id] = []
                the_ref[ans_id].append(ref_ans)
                one_rouge, _ = Rouge().compute_score(the_ref, the_ans)
                rouges.append(one_rouge)
                _, bleu_value = Bleu(bleu_order).compute_bleu_score(the_ref, the_ans)
                bleus.append(bleu_value[3][0])

            the_mat.append(str(np.sum(rouges)))
            the_mat.append(str(np.mean(rouges)))
            the_mat.append(str(np.max(rouges)))
            the_mat.append(str(np.min(rouges)))

            the_mat.append(str(np.sum(bleus)))
            the_mat.append(str(np.mean(bleus)))
            the_mat.append(str(np.max(bleus)))
            the_mat.append(str(np.min(bleus)))

            result.append({'question_id': key, "ans_id": str(ans_id), 'the_mat': the_mat})
